 ...
 */
static uint8_t RFBuffer[RF_BUFFER_SIZE];		//RFÊÕ·¢»º³åÇø
...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_INIT:
        ...
        PacketTimeout = ( uint16_t )( round( ( ( double )8.0 * ( ( double )SX1276FskGetPacketPayloadSize( ) ) / ( double )FskSettings.Bitrate ) * 1000.0 ) + 1.0 );
