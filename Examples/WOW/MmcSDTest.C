...
//  
  ...
  LCD_Printf (0,96,32,0,"图片显示测试");					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
...
void SD_Configuration(void)
{
  ...
  if(0  ==  result)
  {
    ...
    //========================得到容量信息，包括总容量和剩余容量
    SD_disk_getcapacity("0",&sd_size,&free);
    LCD_Printf (0,48,16,0,"SD卡容量%dKByte,%dMB,%0.5fGB",sd_size,sd_size/1024,(double)(sd_size/1024)/1024);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
    //========================获取剩余容量    
    LCD_Printf (0,64,16,0,"SD卡剩余容量%dKByte,%dMB,%0.5fGB",free,free/1024,(double)(free/1024)/1024);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
    free  = sd_size-free;
    LCD_Printf (0,80,16,0,"已用容量%dKByte,%dMB,%0.5fGB",free,free/1024,(double)(free/1024)/1024);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
  }
  else
  {
