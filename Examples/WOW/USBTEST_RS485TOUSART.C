...
#ifdef USBTEST_RS485TOUSART			//小推车主控板
...
RS485_TypeDef RS4851;
void USBTEST_RS485TOUSART_Conf(void);
...
* 返回 		:
*******************************************************************************/
void USBTEST_RS485TOUSART_Configuration(void)
{
	SYS_Configuration();							//系统配置---打开系统时钟 STM32_SYS.H
	
	GPIO_DeInitAll();									//将所有的GPIO关闭----V20170605
	
	USBTEST_RS485TOUSART_Conf();
...
* 返回 		:
*******************************************************************************/
void USBTEST_RS485TOUSART_Server(void)
{	
	IWDG_Feed();								//独立看门狗喂狗
	RuningTime++;
	if(RuningTime>=200)				//1秒
	{
		...
		if(Tcnt>=1000)
			Tcnt=0;
		RS485_DMAPrintf(&RS4851,"printf22-%d",Tcnt);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
