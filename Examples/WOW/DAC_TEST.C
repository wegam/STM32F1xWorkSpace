...
* 返回 		:
*******************************************************************************/
void DAC_TEST_Configuration(void)
{
...
//	DAC_DMA_Configuration((u32*)&ADCD);
...
* 返回 		:
*******************************************************************************/
void DAC_TEST_Server(void)
{
	SYSTIME	++;
//	DAC_SetChannel1Data(DAC_Align_12b_R, SYSTIME*40);
//	DAC_SoftwareTriggerCmd(DAC_Channel_1,ENABLE);
...
//	USART_DMAPrintf(USART1,"AD值：%04d;\t设定值：%04d\n",tempr,SYSTIME*40);
	...
	if(SYSTIME	>=	500)
	{
		...
		tempr	=	ADC_GetConversionValue(ADC1);
		Temperature	=	Get_ADC_Temperature(tempr);
		USART_DMAPrintf(USART1,"AD值：%04d;\t电压值%6.3fV;\t温度%6.3f℃\n",tempr,(float)tempr*3.3/4095.0,Temperature);
