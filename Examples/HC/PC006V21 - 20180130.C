...
void PC006V21_Configuration(void)
{
	...
	{		
		MotorRLBoard_Configuration();					//旋转电机控制板配置
		PWM_OUT(TIM2,PWM_OUTChannel1,SYSLED_FRQ,500);			//SYS-LED 1HZ 50%		SYSLED_FRQ
	}
	else
	{
...
void SWITCHID_Configuration(void)			//拔码开关初始化及读数
{
	...
	//==========================升降电机控制板
	if(SWITCHID_DATA.SW6==1&&SWITCHID_DATA.SW5==1&&SWITCHID_DATA.SW4==0)
	{
		MotorUD		=	1;	//升降电机控制板标志，0-非升降电机控制，1-升降电机控制
	}
	//==========================旋转电机控制板
	if(SWITCHID_DATA.SW6==1&&SWITCHID_DATA.SW5==0&&SWITCHID_DATA.SW4==1)
	{
		MotorRL		=	1;		//旋转电机控制板标志，0-非电机控制板，1-电机控制板
	}
	//==========================旋转电机传感器板
	if(SWITCHID_DATA.SW6==1&&SWITCHID_DATA.SW5==0&&SWITCHID_DATA.SW4==0)
	{
...
//	u8 status	=	0;
	...
	if((MOTOR_TIMxRL->SR & TIM_IT_Update)==TIM_IT_Update	&&	RunToWindow!=0)			//表示定时中断---运行计数未完
	{
		if(PlusFlg	!=0)			//步进电机脉冲标志，一个上升沿计一个脉冲
		{
			...
			if((RxMessage.StdId==SensorRL_ID)&&(RxMessage.Data[1]!=0)&&(RunToWindow==1))
			{
				...
				MOTOR_PlusLow;		//MOTOR_Plus输出关闭（低电平）
				TIM_Cmd(MOTOR_TIMxRL, DISABLE); 	//关闭定时器
			}
			else if(SteepCount>=SteepNeedToRun)	//运行步数到达
			{
				StatusOfWindow	=	RunToWindow;	//新的电机停止窗口位
			...
			else
			{
				...
				{
					if(SteepCount<PWM_RunUpCountRL)
					{
						RunFrequency	+=	PWM_UpdataCountRL;		//当前运行频率
						TIM_SetFreq(MOTOR_TIMxRL,RunFrequency);		//设定频率
					}
					else if(SteepCount+PWM_RunUpCountRL>=SteepNeedToRun)
					{
						RunFrequency	-=	PWM_UpdataCountRL;		//当前运行频率
						TIM_SetFreq(MOTOR_TIMxRL,RunFrequency);		//设定频率
					}
		...
		TIM_ClearITPendingBit(MOTOR_TIMxRL, TIM_IT_Update);			//清除中断标志
	}
	...
	else if(StatusOfWindow!=0	&&	RunToWindow==0	&&	CMDOfWindow!=0)		//电机在待机待机状态下有新的窗口命令
	{
		...
		else if(CMDOfWindow	>	StatusOfWindow)
		{
			...
			if(RunToWindow	==	1)
			{
				SteepNeedToRun		=	(RunToWindow-StatusOfWindow)*SteepPerWindowRL+PWM_RunUpCountRL;				//需要运行的总步数
			}
			else
			{
				SteepNeedToRun		=	(RunToWindow-StatusOfWindow)*SteepPerWindowRL;				//需要运行的总步数
...
void Switch_Server(void)			//检查拔码地址有无变更，如果变更，重新配置运行参数
{
	...
	else if((SWITCHID.nSWITCHID&0x3F)	==	0x00)		//拨码未拨，循环配置
	{
		PC006V21_Configuration();
	}
}
