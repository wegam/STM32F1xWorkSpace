...
void PC016V20_Configuration(void)
{
	...
	USART_DMASend	(ICCardReadPort1,CardReaderCmd_SetArea,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	USART_DMASend	(ICCardReadPort2,CardReaderCmd_SetArea,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	USART_DMASend	(ICCardReadPort3,CardReaderCmd_SetArea,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	USART_Send		(ICCardReadPort4,CardReaderCmd_SetArea,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	SysTick_DeleymS(100);					//SysTick延时nmS
	//========================上电读卡
	
	USART_DMASend	(ICCardReadPort1,CardReaderCmd_ReadData,14);	//串口DMA发送程序

	USART_DMASend	(ICCardReadPort2,CardReaderCmd_ReadData,14);	//串口DMA发送程序

	USART_DMASend	(ICCardReadPort3,CardReaderCmd_ReadData,14);	//串口DMA发送程序

	USART_Send		(ICCardReadPort4,(u8*)CardReaderCmd_ReadData,14);			//串口5发送程序
	
	SysTick_DeleymS(100);					//SysTick延时nmS
	//写卡
	USART_DMASend	(ICCardReadPort1,CardReaderCmd_WriteData,sizeof(CardReaderCmd_WriteData));	//串口DMA发送程序
...
void MessageServer(sBoradDef*	sBorad)				//外部总线消息处理
{
	u16 Num	=	0;
	//=====================================检查外部总线是否有命令
	Num	=	RS485_ReadBufferIDLE(&RS485_Bus,(u8*)&(sBorad->sPlu.BusData.sFarmeRev));	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer
...
//	RS485BusTxd[7]	=	BCC8((u8*)&RS485BusTxd[1],6);		//8位数异或校验码：地址段+异常码+数据段校验
	...
	RS485_DMASend(&RS485_Bus,RS485BusTxd,9);	//RS485-DMA发送程序
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	function
...
//	}
	...
	RS485_DMASend(&RS485_Bus,RS485BusTxd,6+RS485BusTxd[6]+3);	//RS485-DMA发送程序
}
//======
...
void CardReaderSendData(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize)		//向读卡器发送数据
{
	...
	{
		USART_Send		(USARTx,tx_buffer,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	}
	else
	{
		USART_DMASend	(USARTx,tx_buffer,14);	//设置读头读卡扇区、块号、KEYA/KEYB、读卡密码0x44
	}
}
...
u16 CardReaderReadData(USART_TypeDef* USARTx,u8 *RevBuffer,u8 *RxdBuffer)			//检查读卡器有无收到数据
{
	...
	{
		Num	=	USART_ReadBufferIDLE(USARTx,RevBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer
	}
	return Num;
}
...
void HALUsartRemapDisable(void)												//关闭串口复用
{
	...
	USART_DMA_ConfigurationNR	((sBorad.Port2.sReader.UartPort),ICCardReadBaudRate,CardDataSize);	//USART_DMA配置--查询方式，不开中断
}
...
{
//	HALUsartRemapEnable();											//使能串口复用
	RS485_DMASend(&RS485_Seg7,(u8*)Buffer,Length);	//RS485-DMA发送程序
}
//======
...
void RS485_Configuration(void)			//RS485配置
{
	...
	RS485_DMA_ConfigurationNR	(&RS485_Bus,Bus485BaudRate,Bus485BufferSize);	//USART_DMA配置--查询方式，不开中断,配置完默认为接收状态
...
{
	USART_DMA_ConfigurationNR	((sBorad.Port1.sReader.UartPort),ICCardReadBaudRate,CardDataSize);	//USART_DMA配置--查询方式，不开中断
	USART_DMA_ConfigurationNR	((sBorad.Port2.sReader.UartPort),ICCardReadBaudRate,CardDataSize);	//USART_DMA配置--查询方式，不开中断
	USART_DMA_ConfigurationNR	((sBorad.Port3.sReader.UartPort),ICCardReadBaudRate,CardDataSize);	//USART_DMA配置--查询方式，不开中断
