...
HCResult APIRS232UplinkSetData(const unsigned char *buffer,const unsigned short length)
{
	...
	RS485FrameDef *RS485Frame 		= NULL;	
	DataHeadDef		*RS232DataHead	=	NULL;
	DataHeadDef		*RS485DataHead	=	NULL;
  ...
  //=========================================查找RS485HeadCode头标识符
	...
	{		
		...
		//=========================================拷贝相关数据
		//-----------------------------------------本地消息：单元板地址为0
		RS232DataHead	=	&RS232Frame->DataHead;
		if((0==RS232DataHead->Addr.CabinetAddr)&&(DataLength<BusDataSize))
		{
			for(i=0;i<SelfBuffSize;i++)
			{
				if(0	==	ProcessNode[i].Length)
		...
		else
		{
			...
			for(i=0;i<MaxNetPerLayer;i++)
			{
				if(0	==	DataNodeD[i].Length)
				{
					...
					RS485DataHead	=	&RS485Frame->DataHead;
					
					memcpy(RS485DataHead,RS232DataHead,sizeof(DataHeadDef));
					
					RS485Frame->HeadCode		=	RS485HeadCode;
					RS485Frame->TargetAddr	=	RS232DataHead->Addr.CabinetAddr;
					RS485Frame->SourceAddr	=	0x00;
					
					RS485Frame->DataLength	=	RS232Frame->DataLength-4+3;
					//---------------获取数据地址
					pTemp	=	(unsigned char*)&RS232DataHead->ErrCode;
					pTemp++;
