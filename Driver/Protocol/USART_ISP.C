...
{
	USART_DMA_ConfigurationEV	(ISP_Conf->USARTx,115200,ISP_BufferSize);	//USART_DMA配置--查询方式，不开中断
...
{
	ISP_Conf->ISP_DATA.ReceivedLen=USART_ReadBufferIDLE(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer
...
void Usart_ISP_ACK(ISP_Conf_TypeDef *ISP_Conf)	//ISP应答
{
	...
	api_usart_dma_send(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.Command,1);	//串口DMA发送程序
}
/*******************************************************************************
* 函数名			:	Usart_ISP_NACK
* 功能描述		:	ISP不应答
...
//	unsigned short rxNum=0;
	...
	if(C0==ISP_COMMAND_Get)			//获取当前自举程序版本及允许使用的命令
	{
		...
		api_usart_dma_send(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.USARTSendLen);	//串口DMA发送程序	
...
//		api_usart_dma_send(ISP_Conf->USARTx,(u32*)ISP_Conf->ISP_DATA.ISP_TxBuffer,rxNum);	//串口DMA发送程序	
//	}
}
/*******************************************************************************
...
{	
	...
	api_usart_dma_send(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.USARTSendLen+2);				//串口DMA发送程序
...
void Usart_MISP_CommandSend(ISP_Conf_TypeDef *ISP_Conf,unsigned char Command)	//串口编程发送命令程序
{
	...
	api_usart_dma_send(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.Command,2);	//串口DMA发送程序
}
/*******************************************************************************
...
{
	ISP_Conf->ISP_DATA.ReceivedLen=USART_ReadBufferIDLE(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的
	if(ISP_Conf->ISP_DATA.ReceivedLen)
	{
...
{
	USART_DMA_ConfigurationEV	(BSP_Conf->BSP_Port.USARTx,115200,ISP_BufferSize);	//USART_DMA配置--查询方式，不开中断
...
void BSP_SendBuffer(BSP_Conf_TypeDef *BSP_Conf)						//通过串口发送数据，当检测到USARTSendLen不为0时自动发送
{
	if(BSP_Conf->BSP_DATA.USARTSendLen!=0)
	{
		...
		api_usart_dma_send(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.BSP_TxBuffer,BSP_Conf->BSP_DATA.USARTSendLen);		//串口DMA发送程序
...
void USM_BSP_GetAck(BSP_Conf_TypeDef *BSP_Conf)		//获取从机应答
{
	...
	RxNum=USART_ReadBufferIDLE(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.BSP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的
...
void USM_BSP_SendCommand(BSP_Conf_TypeDef *BSP_Conf,BSP_COMMAND_TypeDef Command)	//串口编程发送命令程序
{
	...
	api_usart_dma_send(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.Command,2);	//串口DMA发送程序
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	发送函数包括从机发送应答、发送地址、发送命令、发送数据
