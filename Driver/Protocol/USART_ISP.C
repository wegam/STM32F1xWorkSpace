...
void Usart_ISP_ACK(ISP_Conf_TypeDef *ISP_Conf)	//ISP应答
{
	...
	USART_DMASend(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.Command,1);	//串口DMA发送程序
}
/*******************************************************************************
* 函数名			:	Usart_ISP_NACK
* 功能描述		:	ISP不应答
...
//	unsigned short rxNum=0;
	...
	if(C0==ISP_COMMAND_Get)			//获取当前自举程序版本及允许使用的命令
	{
		...
		USART_DMASend(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.USARTSendLen);	//串口DMA发送程序	
...
{	
	...
	USART_DMASend(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.USARTSendLen+2);				//串口DMA发送程序
...
void Usart_MISP_CommandSend(ISP_Conf_TypeDef *ISP_Conf,unsigned char Command)	//串口编程发送命令程序
{
	...
	USART_DMASend(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.Command,2);	//串口DMA发送程序
}
/*******************************************************************************
...
void BSP_SendBuffer(BSP_Conf_TypeDef *BSP_Conf)						//通过串口发送数据，当检测到USARTSendLen不为0时自动发送
{
	if(BSP_Conf->BSP_DATA.USARTSendLen!=0)
	{
		...
		USART_DMASend(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.BSP_TxBuffer,BSP_Conf->BSP_DATA.USARTSendLen);		//串口DMA发送程序
...
void USM_BSP_SendCommand(BSP_Conf_TypeDef *BSP_Conf,BSP_COMMAND_TypeDef Command)	//串口编程发送命令程序
{
	...
	USART_DMASend(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.Command,2);	//串口DMA发送程序
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	发送函数包括从机发送应答、发送地址、发送命令、发送数据
