			...
			if(HCSYS.Layer<3)		//非最终端设备
			{
				...
				if(0xFF	==	NextAddr)
				{
					...
					else
					{
						...
						DataNodeD[0].NodeLength	=	RS485Frame->DataLength+RS485StartFrameLen+2;						
				...
				else
				{
					...
					else
					{
						...
						DataNodeD[NextAddr].NodeLength	=	RS485Frame->DataLength+RS485StartFrameLen+2;
...
//						memcpy(ProcessNode[i].data,RS485Frame,RS485Frame->DataLength+RS485StartFrameLen+2);	//将源数据全部拷贝
						...
						ProcessNode[i].NodeLength	=	RS485Frame->DataLength;
						break;
					}
				}
...
//				SourceAddr	=	Addr[HCSYS.Layer];
				...
				else
				{
					...
					DataNodeU[SourceAddr].NodeLength	=	RS485Frame->DataLength+RS485StartFrameLen+2;
...
//      pBuffer     = (unsigned char*)RS485Frame->data;
      ...
      SendNode[i].NodeLength  = RS485Frame->DataLength+RS485StartFrameLen+2;
