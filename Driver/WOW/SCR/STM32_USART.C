...
//--------USART全局变量定义
#define	uRxSize		512			//默认串口DMA接收缓冲大小
		...
		case 	USART1_BASE:
			...
			case 	USART2_BASE:
					if(USART_GetITStatus(USART2,USART_IT_IDLE)||USART_GetFlagStatus(USART2,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA1_Channel6->CMAR=(u32)uRx1Addr;							//重新设置DMA接收地址
			...
			case 	USART3_BASE:
					if(USART_GetITStatus(USART3,USART_IT_IDLE)||USART_GetFlagStatus(USART3,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA1_Channel3->CMAR=(u32)uRx1Addr;							//重新设置DMA接收地址
			...
			case 	UART4_BASE:
					if(USART_GetITStatus(UART4,USART_IT_IDLE)||USART_GetFlagStatus(UART4,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA2_Channel3->CMAR=(u32)uRx1Addr;							//重新设置DMA接收地址
