...
//	DMAPrintf_Buffer = (char*)malloc(sizeof(char) * num);	
  ...
  BufferSize=api_usart_dma_send(USARTx,(u8*)DMAPrintf_Buffer,BufferSize);	//串口DMA发送程序
...
*******************************************************************************/
static unsigned short set_usart_dma_buffer(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize)		//串口DMA发送程序
{
	switch(*(u32*)&USARTx)
	{
		case USART1_BASE:
...
unsigned short api_usart_dma_send(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize)		//串口DMA发送程序
{
	if(0==get_usart_tx_idle(USARTx))		//串口状态检查
		return 0;
	return set_usart_dma_buffer(USARTx,tx_buffer,BufferSize);		//串口DMA发送程序	
}
/*******************************************************************************
*函数名			:	USART_DMASend
...
*返回值			:	如果数据已经传入到DMA，返回Buffer大小，否则返回0（发送器忙）
*******************************************************************************/
u16 USART_DMASendList(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize)		//串口DMA发送程序
{
...
static unsigned char get_usart1_rx_idle(void)
{
	...
	if(USART_GetFlagStatus(USART1,USART_FLAG_TC))		//检测发送数据寄存器是否为空	RESET-非空，SET-空，
	{
		rxdelay	=	0;
		return 1;			//空闲
	}
...
static unsigned char get_usart1_tx_idle(void)
{
	...
	BufferSize	=	DMA1_Channel4->CNDTR;		      //获取DMA发送缓存剩余空间
	//发送状态检查
	if((BufferSize!=0)&&((DMA1_Channel4->CCR&0x00000001)!=0))   //还在发送中
	{
	}
	else
	{
...
static unsigned char get_usart2_rx_idle(void)
{
	unsigned short	BufferSize	=	0;			//剩余缓存大小
	static unsigned char rxdelay=0;
	if(USART_GetFlagStatus(USART2,USART_FLAG_TC))		//检测发送数据寄存器是否为空	RESET-非空，SET-空，
	{
		rxdelay	=	0;
		return 1;			//空闲
	}
	...
	if(BufferSize<SetDmaSize.nUSART2)	          //可用缓存小于预设大小，表示已使用
	{
		if(RemaDmaSize.nUSART2	== BufferSize)    //查询可用缓存大小是否继续在变化从而判断是否还在接收中
		{
			if(rxdelay++>=5)               //连续5次查询未变化表示接收完成
			{
				rxdelay	=	0;
		...
		{
			rxdelay	=	0;
			RemaDmaSize.nUSART2		=	BufferSize;   	//更新
		}					
	}
	else
	{
...
static unsigned char get_usart2_tx_idle(void)
{
	...
	BufferSize	=	DMA1_Channel7->CNDTR;		      //获取DMA发送缓存剩余空间
	//发送状态检查
	if((BufferSize!=0)&&((DMA1_Channel7->CCR&0x00000001)!=0))   //还在发送中
	{
	}
	else
	{
...
static unsigned char get_usart3_rx_idle(void)
{
	unsigned short	BufferSize	=	0;			//剩余缓存大小
	static unsigned char rxdelay=0;
	if(USART_GetFlagStatus(USART3,USART_FLAG_TC))		//检测发送数据寄存器是否为空	RESET-非空，SET-空，
	{
		rxdelay	=	0;
		return 1;			//空闲
	}
	...
	if(BufferSize<SetDmaSize.nUSART3)	          //可用缓存小于预设大小，表示已使用
	{
		if(RemaDmaSize.nUSART3	== BufferSize)    //查询可用缓存大小是否继续在变化从而判断是否还在接收中
		{
			if(rxdelay++>=5)               //连续5次查询未变化表示接收完成
			{
				rxdelay	=	0;
		...
		{
			rxdelay	=	0;
			RemaDmaSize.nUSART3		=	BufferSize;   	//更新
		}					
	}
	else
	{
...
static unsigned char get_usart3_tx_idle(void)
{
	...
	BufferSize	=	DMA1_Channel2->CNDTR;		      //获取DMA发送缓存剩余空间
	//发送状态检查
	if((BufferSize!=0)&&((DMA1_Channel2->CCR&0x00000001)!=0))   //还在发送中
	{
	}
	else
	{
...
static unsigned char get_uart4_rx_idle(void)
{
	unsigned short	BufferSize	=	0;			//剩余缓存大小
	static unsigned char rxdelay=0;
	if(USART_GetFlagStatus(UART4,USART_FLAG_TC))		//检测发送数据寄存器是否为空	RESET-非空，SET-空，
	{
		rxdelay	=	0;
		return 1;			//空闲
	}
	...
	if(BufferSize<SetDmaSize.nUART4)	          //可用缓存小于预设大小，表示已使用
	{
		if(RemaDmaSize.nUART4	== BufferSize)    //查询可用缓存大小是否继续在变化从而判断是否还在接收中
		{
			if(rxdelay++>=5)               //连续5次查询未变化表示接收完成
			{
				rxdelay	=	0;
		...
		{
			rxdelay	=	0;
			RemaDmaSize.nUART4		=	BufferSize;   	//更新
		}					
	}
	else
	{
...
static unsigned char get_uart4_tx_idle(void)
{
	...
	BufferSize	=	DMA2_Channel5->CNDTR;		      //获取DMA发送缓存剩余空间
	//发送状态检查
	if((BufferSize!=0)&&((DMA2_Channel5->CCR&0x00000001)!=0))   //还在发送中
	{
	}
	else
	{
...
*注释				:	wegam@sina.com
*******************************************************************************/
static unsigned char get_usart_rx_idle(USART_TypeDef* USARTx)		//串口状态检查
{
	if(USART1 == USARTx)
...
*注释				:	wegam@sina.com
*******************************************************************************/
static unsigned char get_usart_tx_idle(USART_TypeDef* USARTx)		//串口状态检查
{
  if(USART1 == USARTx)
...
)	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer，
{
	...
	if(get_usart_rx_idle(USARTx))		//bit[0] 0-串口空闲；1-串口非空闲，状态根据以下位定义
	{
		RS485_RX_EN(pRS485);
...
//	u32	DMA_status=0;			//DMA状态
  unsigned short sendedlen  =0;
	USART_TypeDef* USARTx	=	pRS485->USARTx;
	if(get_usart_rx_idle(USARTx)&&get_usart_tx_idle(USARTx))		//bit[0] 0-串口空闲；1-串口非空闲，状态根据以下位定义
	{
		RS485_TX_EN(pRS485);
		sendedlen = set_usart_dma_buffer(USARTx,(u8*)tx_buffer,BufferSize);		//串口DMA发送程序
	}
	return sendedlen;
}
