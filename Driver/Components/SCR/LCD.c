...
//		va_end( ap );
	...
	{ 
		...
		if(dst>0x80)
		{
			...
			GetBufferLength	=	GT32L32_ReadBuffer(&LCDSYS->GT32L32,font,word);		//从字库中读数据并返回数据长度
			//写入屏幕
		...
		{			
			...
			GetBufferLength	=	GT32L32_ReadBuffer(&LCDSYS->GT32L32,font,(u16)dst);		//从字库中读数据并返回数据长度
//			//写入屏幕
