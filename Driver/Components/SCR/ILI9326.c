...
void ILI9326_WriteIndex(unsigned int Index)	//写16位命令索引编号
{
	...
	ILI9326_DATABUS_PORT->ODR = Index;
	ILI9326_WR_LOW;
	ILI9326_WR_HIGH;
	ILI9326_CS_HIGH;
}
...
{
	ILI9326_WriteIndex(addr);	//写16位命令索引编号

	ILI9326_WriteData(Data);	//写数据
}
/**************************************************************************************************
...
void ILI9326_CHINESE(unsigned char zimo[720],unsigned int backcolor)  //写入字符
{
	...
	{  
		for(j=0;j<=7;j++)
		{
			if((*point<<j)&0x80)
			{
				ILI9326_WriteData(colors[flag]);
				ILI9326_Delay(1);
			}
			else
			{
				ILI9326_WriteData(backcolor);
				ILI9326_Delay(1);
			}
		}   
		if(i%72==0)
...
//	SSD1963_WR_Write; 		//0--写开启，1--写关闭
	...
	{ 
		...
		for(j=0;j<8;j++)
		{
			...
			ILI9326_WriteData(ILI9326_PEN_COLOR);
//			SSD1963_WR_Write; 		//0--写开启，1--写关闭
