  ...
  //========================发送复位命令进入IDLE状态
    ...
    //------------------------检查是否为SD V2.0
        ...
        buf[i]=SD_SPI_ReadWriteByte(0XFF);	//Get trailing return value of R7 resp
			...
			{
				retry=0XFFFE;
        //------------------------循环发送CMD55和CMD41直到返回0x00，使SD卡进入Ready状态
				do
				{          
        ...
        //------------------------发送CMD58获取OCR信息
            ...
            buf[i]=SD_SPI_ReadWriteByte(0XFF);    //得到OCR值
					if(buf[0]&0x40)
            SD_Type=SD_TYPE_V2HC;     //大容量V2.0SD卡 
					else
            SD_Type=SD_TYPE_V2MC;     //小容量V2.0SD卡 
				}
			}
		}
    else//SD V1.x/ MMC	V3
