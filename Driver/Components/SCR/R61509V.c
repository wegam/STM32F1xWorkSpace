...
R61509VDef	*pR61509V	=	0;
LcdDisplayDef	LcdDisplay;
...
=======
	...
	LcdDisplay.WriteIndexStart	=	R61509V_WriteIndexStart;
	LcdDisplay.WriteIndexEnd		=	R61509V_WriteIndexEnd;
	LcdDisplay.WriteDataStart		=	R61509V_WriteDataStart;
	LcdDisplay.WriteDataEnd			=	R61509V_WriteDataEnd;
	LcdDisplay.WriteData				=	R61509V_WriteData;
	
	LcdDisplay.WriteCommand	=	R61509V_WriteCommand;
	LcdDisplay.WriteAddress	=	R61509V_SetWindowAddress;
	
	LcdDisplay.DSPClean				=	R61509V_Clean;
	LcdDisplay.DSPDrawDot			=	R61509V_DrawDot;
	LcdDisplay.DSPDrawLine		=	R61509V_DrawLine;
	
	
	pInfo->Data.MaxH	=	240;
	pInfo->Data.MaxV	=	400;
	pInfo->Data.Rotate	=	Draw_Rotate_90D;
...
void R61509V_WriteIndexEnd( void )
{
	R61509V_CS_HIGH;	//片选信号 0:片选;1:失能;
}
/**************************************************************************************************
* [Function] R61509V_DispOff:  关闭R61509V显示( 黑屏?)
* [No param]
...
=======
	...
	eRotate	Rotate	=	pR61509V->Data.Rotate;

	Command	=	LcdDisplay.WriteCommand;
...
else //(Rotate	==Draw_Rotate_270D)
{
	...
	Command(R61509V_R210_HSA,HSX);		//Window Horizontal RAM Address Start(R210h)		//水平
	Command(R61509V_R211_HEA,HEX);		//Window Horizontal RAM Address End(R211h)			//水平
	Command(R61509V_R212_VSA,HSY);		//Window Vertical RAM Address Start (R212h)			//垂直
	Command(R61509V_R213_VEA,HEY);		//Window Vertical RAM Address End (R213h)				//垂直
	
	Command(R61509V_R200_HA,HSX);			//RAM Address Set (Horizontal Address) (R200h)
	Command(R61509V_R201_VA,HSY);			//RAM Address Set (Vertical Address) (R201h)

	Command(R61509V_R003_EM,Model);						//RAM Address Set (Vertical Address) (R201h)
	R61509V_WriteIndex16(R61509V_R202_GDRW);								//GRAM(Graphics RAM--图形内存) Data Write (R202h)准备写入
...
<<<<<<< HEAD
	...
	LCD_BL_ON;		//开背光
=======
	Command(R61509V_R000_IR		,	0x0000);		R61509V_Delay(dtime); //四次写0x00
	Command(R61509V_R000_IR		,	0x0000);		R61509V_Delay(dtime); //四次写0x00
	Command(R61509V_R000_IR		,	0x0000);		R61509V_Delay(dtime); //四次写0x00
	Command(R61509V_R000_IR		,	0x0000);		R61509V_Delay(dtime); //四次写0x00
	
	Command(R61509V_R001_DOC		,	0x0300);		R61509V_Delay(dtime); //--------显示方向控制When SS = “1” and BGR = “1”, RGB dots are assigned one to one from S720 to S1.
	Command(R61509V_R002_DWC		,	0x0100);		R61509V_Delay(dtime); //--------	
	Command(R61509V_R003_EM		,	0X5030);		R61509V_Delay(dtime); //--------RGB设为5-6-5模式------------0X5030
	Command(R61509V_R007_DC1		,	0x0100);		R61509V_Delay(dtime); //--------测试显示控制base image的关与开
	Command(R61509V_R008_DC2		,	0X0808);		R61509V_Delay(dtime); //--------
	Command(R61509V_R009_DC3		,	0x0001);		R61509V_Delay(dtime); //--------	
	
	Command(R61509V_R00B_ECC		,	0x0001);		R61509V_Delay(dtime); //--------
	Command(R61509V_R00C_EDIC1	,	0x0000);		R61509V_Delay(dtime); //--------
	Command(R61509V_R00F_EDIC2	,	0x0000);		R61509V_Delay(dtime); //--------
	
	Command(R61509V_R010_PIC1	,	0x0019);		R61509V_Delay(dtime); //--------面板频率--0x19
	Command(R61509V_R011_PIC2	,	0x0101);		R61509V_Delay(dtime); //--------		Panel Interface Control 2 (R011h)
	Command(R61509V_R012_PIC3	,	0x0000);		R61509V_Delay(dtime); //--------
	Command(R61509V_R013_PIC4	,	0x0001);		R61509V_Delay(dtime); //--------
	Command(R61509V_R014_PIC5	,	0x0055);		R61509V_Delay(dtime); //--------
	Command(R61509V_R020_PIC6	,	0x0010);		R61509V_Delay(dtime); //--------
	Command(R61509V_R021_PIC7	,	0x0101);		R61509V_Delay(dtime); //--------
	Command(R61509V_R022_PIC8	,	0x0000);		R61509V_Delay(dtime); //--------
	Command(R61509V_R023_PIC9	,	0x0001);		R61509V_Delay(dtime); //--------	
	
	Command(R61509V_R090_FMC		,	0x8000);		R61509V_Delay(dtime); //--------窗特征控制，FMARK interface
	
	Command(R61509V_R100_PC1		,	0x0330);		R61509V_Delay(dtime); //--------设置电源控制，平衡显示与电消耗
	Command(R61509V_R101_PC2		,	0x0227);		R61509V_Delay(dtime); //--------Power Control 2
	Command(R61509V_R102_PC3		,	0xC1B0);		R61509V_Delay(dtime); //--------PSON,PON 都设为1，电源供应打开
	Command(R61509V_R103_PC4		,	0x0D00);		R61509V_Delay(dtime); //--------	
	
	Command(R61509V_R200_HA			,	0x0000);		R61509V_Delay(dtime);	//--------RAM address 设置
	Command(R61509V_R201_VA			,	0x0000);		R61509V_Delay(dtime);	//--------
	
	Command(R61509V_R280_NVM		,	0xB000);		R61509V_Delay(dtime);	//--------VCM
	
	Command(R61509V_R300_YC1		,	0x0C00);		R61509V_Delay(dtime); //--------
	Command(R61509V_R301_YC2		,	0x5A0B);		R61509V_Delay(dtime); //--------
	Command(R61509V_R302_YC3		,	0x0906);		R61509V_Delay(dtime); //--------
	Command(R61509V_R303_YC4		,	0x1017);		R61509V_Delay(dtime); //--------
	Command(R61509V_R304_YC5		,	0x2300);		R61509V_Delay(dtime); //--------
	Command(R61509V_R305_YC6		,	0x1700);		R61509V_Delay(dtime); //--------
	Command(R61509V_R306_YC7		,	0x6309);		R61509V_Delay(dtime); //--------
	Command(R61509V_R307_YC8		,	0x0C09);		R61509V_Delay(dtime); //--------
	Command(R61509V_R308_YC9		,	0x100C);		R61509V_Delay(dtime); //--------
	Command(R61509V_R309_YC10		,	0x2232);		R61509V_Delay(dtime); //--------
	
	Command(R61509V_R400_BINL		,	0X6400);		R61509V_Delay(dtime); //base image 设置 400线
	Command(R61509V_R401_BIDC		,	0x0004);		R61509V_Delay(dtime); //--------设置base image 滚动模式，开启滚动(颜色反相设置）
	Command(R61509V_R404_BIVSC	,	0x0000);		R61509V_Delay(dtime);	//--------从0线开始滚屏
	
	Command(R61509V_R502_PTEA		,	0x005f);		R61509V_Delay(dtime);
>>>>>>> 24cd2ea4c8c2e2240d51603a2d65a86ccb26281c
}
...
void R61509V_Clean(u16 COLOR)	//清除屏幕函数
{
	...
	LcdDisplay.WriteAddress(HSX,HSY,HEX,HEY);
	LcdDisplay.WriteDataStart();
	for(x=0;x<MaxH;x++)
	{
		for(y=0;y<MaxV;y++)
		{
			LcdDisplay.WriteData(COLOR);
		}
	}	
	LcdDisplay.WriteDataEnd();
}
...
void R61509V_DrawDot(u16 x,u16 y,u16 color)
{
	LcdDisplay.WriteAddress(x,y,x,y);	//设置光标位置 
	LcdDisplay.WriteDataStart();
	LcdDisplay.WriteData(color); 	//笔画颜色
	LcdDisplay.WriteDataEnd();
}
...
)
{
	...
	for(t=0;t<distance+1;t++ )					//画线输出 
	{  
		LcdDisplay.DSPDrawDot(uRow,uCol,color);	//画点 
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
