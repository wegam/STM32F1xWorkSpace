...
void GT32L32_CS_DisSelect(GT32L32Def *GT32L32_Info);
void GT32L32_CS_Select(GT32L32Def *GT32L32_Info);
void GT32L32_CLK_L	(GT32L32Def *GT32L32_Info);
void GT32L32_CLK_H	(GT32L32Def *GT32L32_Info);
void GT32L32_SI_L		(GT32L32Def *GT32L32_Info);
void GT32L32_SI_H		(GT32L32Def *GT32L32_Info);
u8 GT32L32_SO				(GT32L32Def *GT32L32_Info);
...
*返回值			:	无
*******************************************************************************/
void GT32L32_ConfigurationNR(GT32L32Def *pInfo)
{
		//1)**********定义相关结构体
	...
	if(pInfo->Port.sSPIx==SPI1)
	{
		...
		if((pInfo->Port.sGT32L32_CS_PORT==GPIOA)&&(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_4))			//如果SPI_NSS为SPI_NSS_Soft（软件控制方式）
		{
...
//			u8 UseSPI1_flg=0;		//如果使用纯硬件SPI1（含CS脚），UseSPI1_flg=1，否则UseSPI1_flg=0；
			//开CS-GPIO时钟
			if(pInfo->Port.sGT32L32_CS_PORT==GPIOA)
			{
				if((pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_13)||(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_14)||(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_15))
				{
			...
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOB)
			{
				if((pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_3)||(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_4))
				{
			...
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOC)
			{
				if((pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_14)||(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_15))
					RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC |RCC_APB2Periph_AFIO, ENABLE);
				else
					RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);
			}
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOD)
			{
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
			}
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOE)
			{
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE, ENABLE);
			}
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOF)
			{
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF, ENABLE);
			}
			else if(pInfo->Port.sGT32L32_CS_PORT==GPIOG)
			{
			...
			GPIO_Init(GPIOA, &GPIO_InitStructure);
			//CS配置
			GPIO_InitStructure.GPIO_Pin 	= pInfo->Port.sGT32L32_CS_PIN;
			GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_Out_PP;
			GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
			GPIO_Init(pInfo->Port.sGT32L32_CS_PORT, &GPIO_InitStructure);
		}
	}
	else if(pInfo->Port.sSPIx==SPI2)
	{
		...
		if((pInfo->Port.sGT32L32_CS_PORT==GPIOB)&&(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_12))			//如果SPI_NSS为SPI_NSS_Soft（软件控制方式）
		{
//			u8 UseSPI2_flg=1;		//如果使用纯硬件SPI2（含CS脚），UseSPI2_flg=1，否则UseSPI2_flg=0；
			SPI_SSOutputCmd(pInfo->Port.sSPIx, ENABLE);			//如果在主机模式下的片选方式为硬件（SPI_NSS_Hard）方式，此处必须打开，否则NSS无信号
...
//			u8 UseSPI2_flg=0;		//如果使用纯硬件SPI2（含CS脚），UseSPI2_flg=1，否则UseSPI2_flg=0；
	...
	else if(pInfo->Port.sSPIx==SPI3)
	{
		...
		if((pInfo->Port.sGT32L32_CS_PORT==GPIOA)&&(pInfo->Port.sGT32L32_CS_PIN==GPIO_Pin_15))			//如果SPI_NSS为SPI_NSS_Soft（软件控制方式）
		{
	...
	SPI_Init(pInfo->Port.sSPIx,&SPI_InitStructure);

	SPI_Cmd(pInfo->Port.sSPIx, ENABLE);				//使能SPI	
	
	//3)**********使能SPIx_NESS为主输出模式
	if((pInfo->Port.sSPIx->CR1&0X0200)!=SPI_NSS_Soft)						//如果在主机模式下的片选方式为硬件（SPI_NSS_Hard）方式，此处必须打开，否则NSS无信号
	{
		SPI_SSOutputCmd(pInfo->Port.sSPIx, ENABLE);			//如果在主机模式下的片选方式为硬件（SPI_NSS_Hard）方式，此处必须打开，否则NSS无信号
	}
}
...
void GT32L32_CS_DisSelect(GT32L32Def *GT32L32_Info)
{
	GPIO_SetBits(GT32L32_Info->Port.sGT32L32_CS_PORT,	GT32L32_Info->Port.sGT32L32_CS_PIN);			//取消片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	使能片选
...
void GT32L32_CS_Select(GT32L32Def *GT32L32_Info)
{
	GPIO_ResetBits(GT32L32_Info->Port.sGT32L32_CS_PORT,	GT32L32_Info->Port.sGT32L32_CS_PIN);			//使能片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	函数功能说明
...
void GT32L32_CLK_L(GT32L32Def *GT32L32_Info)
{
	GPIO_ResetBits(GT32L32_Info->Port.sGT32L32_CLK_PORT,	GT32L32_Info->Port.sGT32L32_CLK_PIN);			//取消片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	使能片选
...
void GT32L32_CLK_H(GT32L32Def *GT32L32_Info)
{
	GPIO_SetBits(GT32L32_Info->Port.sGT32L32_CLK_PORT,	GT32L32_Info->Port.sGT32L32_CLK_PIN);			//使能片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	函数功能说明
...
void GT32L32_SI_L(GT32L32Def *GT32L32_Info)
{
	GPIO_ResetBits(GT32L32_Info->Port.sGT32L32_MOSI_PORT,	GT32L32_Info->Port.sGT32L32_MOSI_PIN);			//取消片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	使能片选
...
void GT32L32_SI_H(GT32L32Def *GT32L32_Info)
{
	GPIO_SetBits(GT32L32_Info->Port.sGT32L32_MOSI_PORT,	GT32L32_Info->Port.sGT32L32_MOSI_PIN);			//使能片选
}
/*******************************************************************************
*函数名			:	function
*功能描述		:	使能片选
...
u8 GT32L32_SO(GT32L32Def *GT32L32_Info)
{
	return GPIO_ReadOutputDataBit(GT32L32_Info->Port.sGT32L32_MOSI_PORT,GT32L32_Info->Port.sGT32L32_MOSI_PIN);			//读取MISO数据
}
//=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>
...
//->被调用函数	:
//<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=
u8 GT32L32_ReadBufferbac(GT32L32Def *GT32L32_Info,u32 Address,u16 len,u8 *Buffer)
{
	//____________定义变量
	u32 i=0;
//	u8	Status=0XFF;
											...
											)
{
	...
	Address=Address|0x03000000;		//0x03指令字+地址。
//	Address=Address|0x0B000000;//0x0B指令字+地址。--快速
//	STM32_SPI_SendBuffer(SPI1,4,(u8*)&Address);
	GT32L32_ReadWriteByte(Address>>24);				//发送地址高8位
	GT32L32_ReadWriteByte(Address>>16);				//发送地址
	GT32L32_ReadWriteByte(Address>>8);				//发送地址
	GT32L32_ReadWriteByte(Address);						//发送地址
...
//	while((Status=GT32L32_ReadStatus()&0x01)!=0x01);
	...
	for(i=0;i<lengh;i++)
	{
		pInfo->Data.Buffer[i]=GT32L32_ReadWriteByte(0XFF);	//从字库读出点阵数据到数组中。
	}
	//____________取消片选	
	GT32L32_CS_DisSelect(pInfo);
...
//->被调用函数	:
//<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=
void GT32L32_SendByte(GT32L32Def *pInfo,u8 ByteData)
{
	u8 i;
	for(i=0;i<8;i++)
	{
...
//->被调用函数	:
//<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=
u8 GT32L32_ReadByte(GT32L32Def *pInfo)
{
	u8 i;
	u8 dat=0;
	GT32L32_CLK_H(pInfo);
